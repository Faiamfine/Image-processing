...
def distancing(people_coords, img, dist_thres_lim=(200,250)):
    ...
    high_risk = int(list(already_red.values()).count(1))
    low_risk = int(list(already_red.values()).count(0))
...
def plot_results(start=0, stop=0, bucket='', id=(), labels=()):  # from utils.utils import *; plot_results()
    ...
    cv2.rectangle(img, c1, c2, color, thickness=tl, lineType=cv2.LINE_AA)
    cv2.imshow("Social Distance System", img)
